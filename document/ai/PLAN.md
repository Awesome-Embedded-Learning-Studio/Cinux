...
> **现状资产**：Cinux-GUI core host-neutral（子模块 pin `0b0c135` F13-B 收官），`host/linux_fbdev_main.cpp`（用户态 Linux fbdev host）+ evdev + posix_spawn 是 Cinux host 的参考样板；`kernel/gui/host_cinux.cpp` 内核态 adapter（批4 退役）。core **一行不改**就能在用户态跑——Host ABI 是唯一硬缝。
...
- **core host-neutral 不动**：子模块 core 一行不改；批3 只写 Cinux host adapter。
...
| 4a | 4 g++ 冒烟（单线程） | 扩展 extract.sh（cc1plus+libstdc+++C++ headers+libm）；`g++ hello.cpp`；STL+异常试金石；**7 bug 修复链**（do_openat follow symlink / waitpid status 编码 / kprintf %.*s / extract libm self-loop / ProcFs+DevFs lookup_child / assemble 192MB+usr/lib64 / bb 断言解码）；详见 [note](../notes/2026-07-04-f-usability-b4a-gpp-cpp.md) | ✅ 本地 | g++ 闭环 PASS（`Hello from G++!`+`exception: caught`）；两 leg 1108/0 + bb 14/14 + host 69/69；ACT 撞 nektos/act issue 107（container cwd，非脚本），真 CI 待 push |
| 4b | 4 pthread 试金石 | gettid/set_robust_list/sched_getaffinity stub + clone(CLONE_THREAD) 真跑 + pthread create/join/mutex/cond | ⏳ | pthread 闭环 |
...
- **target vs cross 工具链（头号坑）**：进 rootfs 的 GCC 必须是 **native**（在 Cinux 跑、产 Cinux 二进制）。extract.sh 拷的是 host 的 cc1/as/ld——它们是 glibc 动态的 host 二进制，靠一起拷进来的 glibc `.so` + `/lib64/ld-linux` 在 Cinux 跑（已验证）。**绝不能**塞 Buildroot 的 cross compiler（host 程序，Cinux 跑不了，一跑 SIGSEGV 且难分辨是内核还是工具链塞错）。
...
## ✅ F5-M3 NVMe（PCIe SSD）— 收官（已合 main；NVMe 默认 boot disk + AHCI legacy fallback）
...
## ✅ F5-M2/M7 VirtIO 块设备 + 网卡 — 收官（已合 main PR#71；blk+net 真中断+SMP+SLIRP ping+perf）
...
> **2026-07-05 GCC 线阶段收尾**：Cinux 上 gcc/g++ 编译 + 跑用户程序全链路绿。达成：① F12-M2 批4-C2 cc1→as→ld→./hello 自举闭环；② F-USABILITY 批3 gcc driver 单命令 ✅ + 批4a g++ STL+异常 ✅（PR#63/64/65）；③ busybox init PID1（批3b）；④ C 重构治 lto_plugin corrupt（PR#65，refcount/pte_count 分离）；⑤ perf 弧 ext2 agg 降 gcc 编译 I/O 25%（PR#66）；⑥ 默认 PIE gcc/g++ + main-base ASLR（PR#66 PIE B1+B2）；⑦ syscall 补全 gcc/g++ unhandled 90→0（PR#66）。CI 全绿。详见收官 [note](../notes/2026-07-05-gcc-line-finale.md)。

> **open follow-up（非 gcc/g++ 编译必需，登记留后续）**：① pthread（F-USABILITY 批4b，gettid/sched_getaffinity + clone CLONE_THREAD 真跑）；② robust futex 真清理（set_robust_list 现返 0 假成功）；③ sendfile 真实现；④ 199 fremovexattr + xattr 系列；⑤ interpreter ASLR（ldso 固定 USER_INTERP_BASE）；⑥ 静态 PIE；⑦ ext4 rootfs（F6-M5）；⑧ F12-M3 self-hosting（编 Cinux 自己，远期大弧）。
...
> **批4-C1 cc1 --version ✅（2026-07-02 会话四）**：cc1（~47MB GCC C 前端,9 DT_NEEDED：libisl/libmpc/libmpfr/libgmp/libm + as/ld 闭包的 libz/libzstd/libc/ldso）装盘 + `cc1 --version` 跑通。「最不可控」一关首战告捷 —— bug4 的 page-cache/uaccess/ext2 修复扛住 cc1 更重 ldso bring-up + TLS + glibc -O2 构造函数,未暴露新内核 bug。改 `extract.sh`(stage cc1 + ldd 闭包)+ `main_test.cpp`(B4-C1 smoke)。commit `54e61ca`。

> **批4-C2 cc1 编译→as→ld→./hello 全自举闭环 ✅（2026-07-02 会话五）**：Cinux 上 `cc1 /hello.c -o /hello.s` → `as` → `ld` → `./hello` 输出 **"Hello from GCC!"**(Cinux 自己编自己跑的 C 程序)。修 4 个独立 bug:① **cc1 缺 `stdc-predef.h`** —— GCC 隐式 preinclude,`gcc -H` 不列,`extract.sh` 显式 stage hello.c 的 ~25 文件 include 闭包(gcc -H 动态算,~200KB,非全量 249MB /usr/include)。② **`O_TRUNC` 完全没实现** —— cc1 覆写 /hello.o 留旧尾巴(as 读到 `gbits` "no such instruction");加 `InodeOps::truncate`(默认 NotImplemented,ext2 实现:set i_size+persist+VMS size,块释放留 follow-up)+ `do_openat_kernel` O_TRUNC(0x200)调用。③ **Bug A — 内核对 user #PF panic**(该 SIGSEGV):handle_pf 对 user 写只读页等不可解 fault 直接 `panic()`,Linux 投 SIGSEGV 进程;加 user-fault SIGSEGV 路径(kernel fault 才 panic)—— **内核兜底,不再因用户态错误崩**。④ **Bug B — VMA backing inode 悬垂指针(根因)**:VMA 持裸 `Inode*` 无引用计数,ext2 inode_cache_ 驱逐复用 slot 后 VMA 服务到错的文件(ld 跑 cc1 产的 .o 时,某库 .text VMA backing 指向被释放复用的 libc.so 脚本 inode → page cache 返回 EOF 零页 → ld 执行 `00 00`=`add %al,(%rax)` → 写保护 #PF)。修:sys_mmap 设 backing 时 `inode_ref`;vma.cpp 删节点(clear/merge/remove)+ split 复制 backing 时 `inode_ref/unref`;**ext2 驱逐跳过 refcount>0 的 slot**(pinned by fd/VMA)。复用既有 DEBT-023 `Inode::refcount`。两 leg run-kernel-test-all EXIT=0(cc1/as/ld/./hello 各 2/2)+ host 69/0 + 0 panic。⭐**教训**:VMA/File 持 inode 必须引用计数,否则 slab 复用导致 use-after-free(同 page_cache.hpp 注释警告的 key-by-pointer 陷阱,VMA backing 同样适用)。
...
> **批3b（✅ `460b474` + 子模块 Cinux-Base `75df90f`，2026-07-02）**：busybox init 作 PID1。**生产冒烟(build-console 非 GUI)确认** busybox init PID1 + sysinit echo + respawn /bin/sh + ash 提示符(`~ #`) + PID1 reap child（批4 GCC 孤儿基础）。**4 处改动 + 扩展**：① `init.cpp` 入口 alloc PID1（kernel_init 模型，kthread pid=0 不碰 g_pid_alloc，**推翻旧交接"重排 start_poll_driver"误判**）+ usb::init 挪前；② `shell_launch.cpp` 直 execve /sbin/init 不 fork（argv{"init"}）；③ devfs /dev/console 代理 console_tty（ConsoleInput 注入 read+ioctl 保 host-testable）+ console_tty_ioctl 共享（sys_ioctl 复用）+ **Cinux-Base Error 加 Fault(EFAULT)** 根治 copy 失败近似（原 InvalidArgument≈EINVAL 不准；console 有 EFAULT 测试闸）；④ 盘加 /etc/inittab + /sbin/init busybox hardlink。**扩展（生产冒烟暴露）**：补 busybox init 三件套 syscall——vfork(58)=fork / reboot(169)=-EPERM / rt_sigtimedwait(128)=-EAGAIN（驱动 respawn loop；pure block 死锁因 SIGCHLD 被 waitpid 消耗）+ signal_send 加 sigwait waiter 唤醒基建。验证：两 leg run-kernel-test-all 绿 + host 69/69 + EFAULT 测试绿。详见 [note](../notes/2026-07-02-gcc-b3b-busybox-init-pid1.md)。push/PR 归用户。
> **批4 GCC 工具链(glibc 动态)🔄**（分支 `feat/b4-gcc-toolchain` 从 main `0b82e1b`）：**用户决策校正 2026-07-02** — 动态 glibc（兼容最广）+ 不自编工具链（硬盘/必要性）+ 先 as+ld 链路再 cc1 + musl/glibc 共存（首选;内核 PT_INTERP 中立;回退条件=glibc 跑不通）。杠杆：PT_INTERP 是 libc 切换缝,内核动态加载机制复用（F10-M2 验 musl,本弧切 glibc）。**B4-a 第一刀 = as+ld 链路**（hello.s host 预编,省 249MB headers）。**B0 内核 glibc 兼容 4 处 ✅**：auxv +AT_PLATFORM("x86_64")/AT_HWCAP(CPUID.01H:EDX) + execve 扫 PT_GNU_STACK（栈默认 NX,glibc 期望）+ USER_BRK_MAX 64MB→240MB（留 ldso@256MB gap;动态增长留续）。host 69/69 + 两 leg ALL PASSED。vDSO 留 follow-up（glibc fallback syscall）。下：B1 host 提取 as/ld+glibc 运行时 → mkfs.ext2 -d 目录树装盘（debugfs 逐文件退役）。详见 plan `parsed-zooming-fog.md`。
...
> 目标：`gcc hello.c -o hello && ./hello` 在 Cinux 跑通——从「能跑别人二进制」到「能编自己二进制」。ABI 底子已验（busybox -O2 14/14 试金石过）。4 批串行：批1 tmpfs ✅ → 批2 mount/umount2+挂 /tmp ✅ → 批3 sys_access+init PID1 → 批4 GCC 工具链装盘 + gcc hello.c。
...
- **`/proc/<pid>/cmdline` 范围诚实**：Cinux Task 不存 argv，cmdline 返 `name`（尽力，非真 argv）；note 标注。
- **stat 内容是简化版**（非完整 Linux /proc/<pid>/stat 52 字段，Cinux 无 accounting）—— 格式 `pid (name) state ppid tgid uid gid`，note 标注简化。
...
**Cinux 有了高精度 monotonic（HPET）+ 真墙钟（RTC）**：`sys_clock_gettime(CLOCK_MONOTONIC)`
...
### ✅ 收官（2026-06-30）
**Cinux 能跑 musl 动态用户程序**：execve 识别 PT_INTERP → 加载 interp(ldso)到 USER_INTERP_BASE → entry=interp 入口、auxv 喂 AT_BASE/AT_ENTRY/AT_PHDR → musl ldso 在用户态做 GOT/PLT 重定位 + DT_NEEDED + 符号解析 → 跳 AT_ENTRY → write() 经 libc.so 输出。4 commit（批0 docs `bcfebf8` / 批1 kernel `06a7d91`+note `6c4309d` / 批2 toolchain `22aa19e` / 批3 smoke+收官），已合 main PR#49 a134cb7。验证：dyn smoke ON 单核 + -smp2 两 leg 968/0 + 串口 `Hello from musl on Cinux!` ×5 + hello-dyn 5/5 PASS；默认 OFF 零回归。详见 notes `2026-06-30-f10-m2-b{1,2,3}-*.md`。
...
## ✅ F10-M1（用户态运行时 / musl 静态移植）— 收官（已合 main PR#42）
...
| 6 | 端到端：musl hello world 经 execve+ELF loader+auxv 在 QEMU 跑通，printf 输出；加测试项；notes | ✅ | harness ring-3 smoke（`CINUX_MUSL_HELLO_SMOKE`）：950 单测 + 串口 `Hello from musl on Cinux!`（write 路径）+ exit 0 + smoke PASS；默认 OFF 950/0。printf/stdout FILE segfault 留 follow-up（`aad9736`）|
...
### ✅ F10-M1 follow-up（shell 启动 musl + SMP fork）— 多类根因全修（已合 main；GUI shell spawn PR#72 实证 musl/busybox 在 PTY 跑）

> 接批6：GUI shell 敲 /path 启动 musl 程序（fork+execve+waitpid，标准 shell fallback）。**多类根因**：
...
> 2b. **syscall_entry 仍漏恢复 Linux syscall-preserved 参数寄存器**（2026-07-01 F-ECO O2 follow-up）：musl inline `syscall` 只声明 `rax/rcx/r11` 变化，假设 `rdi/rsi/rdx/r10/r8/r9` 跨 syscall 保持；Cinux 入口已保存这些寄存器，但返回路径只恢复 callee-saved，导致 `syscall_dispatch` 的 C 调用 clobber 泄漏回用户态，`-O1/-O2` musl/busybox 复用脏寄存器后堆元数据损坏，最终 mallocng `a_crash` `hlt`。修为 `syscall_entry` + `ret_from_fork` 从 128B frame 恢复 `rdi/rsi/rdx/r10/r8/r9/r12-r15/rbx/rbp`，`rax` 仍为返回值、`rcx/r11` 保持 SYSRET clobber。详见 [note](../notes/2026-07-01-syscall-abi-preserve-user-registers.md)。
> 2c. **syscall/ISR 未保护用户 SIMD/FPU 状态**（2026-07-01 F-ECO O2 真根因）：musl mallocng 在 `__malloc_alloc_meta` 里用 `pxor %xmm0,%xmm0` 生成零向量，跨 syscall/缺页后再 `movups %xmm0,(meta)` 清 `prev/next`；Cinux 进入 kernel C/C++ 前只保存 GPR，允许内核日志/编译器 clobber 用户 `xmm0`，于是 malloc 元数据被写入格式化日志残留并在 `a_crash @ 0x406A66` 触发 `hlt`。修为 syscall 与 ISR 入口先 `fxsave` 用户/被中断 SIMD 到栈上，返回前 `fxrstor`，同时刷新 `Task::fpu_state` 供 fork/preemption 使用；`ret_from_fork` 恢复 child 继承的 SIMD 状态。另补匿名 demand page zero-fill，避免 brk/MAP_ANON/stack 泄漏旧页内容。实测真实 QEMU busybox `echo 5/5 PASS` + `ls PASS`。详见 [note](../notes/2026-07-01-syscall-simd-state-preserve.md)。
...
## ✅ F7 网络协议栈（底子优先：loopback 先 → host 单测 → e1000 收尾）— 收官（已合 main）
...
>
> ✅ **ring3 实跑 #DF 已根治（2026-06-27）**：B1-B3 代码齐 + 内核态全证（test_production_ping/syscall_ping 绿），但 shell 敲 `ping` → sys_ping 里 sti/hlt → 调度器 syscall 中途抢占 → 破坏栈帧 → Double Fault。harness 抓不到（不能真跑 ring3），boot smoke 才暴露。**协议栈 + ping() 本身没问题**。根因 + 修法（阻塞式 ping + 常驻 poll driver，**不要 sti/hlt 自旋**）见 [handoff-df note](../notes/2026-06-26-f7-net-handoff-df.md)。**已合 main**：sys_ping 改可注入 yield 泵 + 常驻 net_poll kthread 替 sti/hlt 自旋（见 note 2026-06-27-sys-ping-df-root-fix）。
...
### ✅ F7-M4 UDP（协议层 + 端口多路复用）— 收官（已合 main PR#47）
...
## ✅ F13-A Cinux-GUI 主体(独立仓 P0-P7)— 跑通(2026-07-05;Cinux 子模块 pin `4c84eb2` 待 bump 到 `7a159c4`)

> GUI 抽成独立跨平台库 **Cinux-GUI**(原 visor 名已废,留给用户嵌入式线;org 仓 `Awesome-Embedded-Learning-Studio/Cinux-GUI` 作 `third_party/Cinux-GUI` submodule)。同一份 core 跑 Cinux 桌面 / Linux(fbdev+evdev)/ SDL / 未来 MCU/GPU。核心机制 = **Host ABI 函数指针表([core/host.hpp](../../third_party/Cinux-GUI/core/host.hpp))唯一硬边界**,换宿主只换表填充。
>
> **Cinux-GUI 仓主体 P0-P7 全跑通**(独立 checkout `~/Cinux-GUI`,main=`7a159c4`,**领先 Cinux 子模块 pin 66 commit**):P0 staging 归 core+swraster+offscreen/replay host → P1 Linux fbdev+evdev host(QEMU 冒烟 PASS)→ P2 Scene+Compositor+帧间 dirty diff → P3 Widget 树(PaintList+Material Theme+控件)→ P4 Window/WindowManager/TerminalWidget+真 shell PTY → P5 字体缩放/flex/per-widget dirty/ANSI 彩色 → P6 TextBox/resize/Scene 退役(Compositor handler OCP)/256 色 → P7 Radio/Dropdown+sdl-host+gallery demo。
>
> **Host ABI 表 P0-P7 零改动**(HostCore 8 回调 + HostDesktop.spawn + Host{core,desktop*,ctx} 全稳定)。变的只是 core 内部:staging 所有权 host→core(`GuiCore` 类,`pump(Host*)`→`GuiCore::pump()` 成员、`Frame.pixels` const→void)、桌面层 Window/WindowManager/TerminalWidget 从 host 搬进 core `widget/` 子树、Scene 在 P6-d 退役 → `Compositor` 类 + `Desktop::render`。
>
> **后续主体**(Cinux-GUI 仓独立开发,用户主导):P8 GPU texture compositor / P9 多进程 Surface 协议 / MCU 线(STM32F1)。详见 [Cinux-GUI 仓 ROADMAP/PLAN/DIRECTIVES](https://github.com/Awesome-Embedded-Learning-Studio/Cinux-GUI)。
>
> **F13 在 Cinux 侧收窄为 host adapter 重写 → 见下面 🔄 F13-B 段。**

---

> **历史进度(2026-06-21→06-23,已合 main,保留作 F13-A 背景):**
>
...
## ✅ F13-B Cinux host adapter 重写接新 core — 收官（已合 main PR#72；切新 core+删老桌面层+桌面图标+IST2 稳定化；后被 F-GUI-USERSPACE 用户态 host 超越，host_cinux.cpp 本批删）

> Cinux 侧 host adapter（[kernel/gui/host_cinux.cpp](../../kernel/gui/host_cinux.cpp)）还停在 2026-06-23 Scene 时代：`render_frame` 调老 `wm.composite()`（P6-d 已退役死路径）、[desktop_launch.cpp](../../kernel/gui/desktop_launch.cpp) 调自由函数 `cinux::gui::pump()`（新 core 已删，改 `GuiCore::pump()` 成员）、子模块 pin 落后 66 commit。老桌面层 6 文件（window/window_manager/terminal/...）+ 老 GUI 测试 ~5849 行，新 core 已 1:1 吸收但 Cinux 侧还留着（跟 Canvas/Pipes/TaskBuilder/launch_user_program/g_pid_alloc 缠绕）。**这是 [Cinux-GUI DIRECTIVES D 段](https://github.com/Awesome-Embedded-Learning-Studio/Cinux-GUI)明说的「Cinux 侧唯一对接点 = host adapter + framebuffer handoff」**。
>
> **用户 2 决策（2026-07-05）**：① **纯渲染优先**（B1a 切新 core，`desktop=NULL` 无终端，B2 单独做 PTY 真终端）；② **这次一起清理**（老桌面层 + 老测试 B1b 删，GUI 净减 ~7300 行）。
>
> **关键利好**：Host ABI 表形态 P0-P7 零改动，host adapter 重写 = 换 `render_frame` 内部调用（`wm.composite()` → `Desktop::render`），不是改 ABI 表；Cinux-GUI 仓 [host/linux_fbdev_main.cpp](https://github.com/Awesome-Embedded-Learning-Studio/Cinux-GUI/blob/main/host/linux_fbdev_main.cpp) 是现成重写模板（fbdev+evdev 同海拔，跟 Cinux VBE+输入最近）。

### 批表
| 批 | 范围 | 状态 | 验证 |
|----|------|------|------|
| 0 | 立项 docs（本段）+ ROADMAP F13 行（F13-A/F13-B）+ todo/f13-gui/ superseded | 🔄 | docs-only |
| 1a | bump pin `4c84eb2`→`7a159c4` + host adapter（[host_cinux.cpp](../../kernel/gui/host_cinux.cpp)）全切新 core（`render_frame`→`Desktop::render`、`dispatch_event`→新 `wm.process_pointer/dispatch_key`、pump→`GuiCore::pump()`，持 HostState=Desktop+WindowManager+GuiCore+Theme+PsfFont）+ desktop_launch/handoff 切 + gui_init 构新 widget 树（`desktop=NULL` 纯渲染）+ freestanding 头门禁验；**老桌面层/老测试留对照** | ⏳ | 两 leg run-kernel-test-all（老测试留）+ GUI 冒烟（新 widget 桌面纯渲染，用户自启） |
| 1b | 删老桌面层 6 文件 + desktop_icon/icon ~1500 行 + 老 GUI 测试 ~5849 行 + kernel/gui/CMakeLists + main_test.cpp wiring；test_terminal_shell/multi_terminal 留（B2 改写测新 TerminalSession）；Canvas 看是否退役 | ⏳ | 两 leg run-kernel-test-all（测试数大降）+ 全量编 + GUI 冒烟回归 |
| 2 | PTY 真终端：kernel-side `TerminalSession`（PTY master inode+shell_pid+新 TerminalWidget*）+ `HostDesktop.spawn` 真实现（fork+execve /bin/sh + 挂 PTY slave，复用 F10-M3 PTY 框架）+ render_frame 每帧 drain PTY→`TerminalWidget::write` + 键盘路由 PTY stdin | ⏳ | GUI 真跑 shell（用户自启验） |
| 3 | 收官：note + ROADMAP F13-B ✅ | ⏳ | docs-only |

### 风险 / 陷阱
- **硬前置捆绑**：bump pin + `render_frame` 切 `Desktop::render` + pump 切 `GuiCore::pump` 三者必须同批（bump 后 `pump.hpp` 没了立刻编不过）。B1a 一口气做。
- **freestanding 头门禁**：新 core `widget/` 子树可能引禁止头（`<memory>` 等，[[kernel-can-use-std-smart-ptr]]），bump 后全量编验证，必要时 per-file EXEMPT 或 host adapter 隔离。
- **新老 WindowManager 类名冲突**：老 `kernel/gui/window_manager.hpp` vs 新 `third_party/Cinux-GUI/core/widget/window_manager.hpp` 可能同名；host_cinux/gui_init 只 include 新 core（不 include 老头）避撞，B1a 先验。
- **GUI 验证用户自启**（[[gui-verification-user-starts-always]]）：Claude 只准备环境（GUI=ON + rebuild + assemble rootfs），不 background 启 GUI QEMU；console gate（run-kernel-test-all）Claude 跑。
- **bump pin 顺序**：submodule pin 改父仓 worktree commit（用户控制 push）；Cinux-GUI 仓 main 已是 `7a159c4`（无需 push 子模块）。
- **`desktop=NULL` 中间态无终端**：B1a/B1b 后 GUI 桌面纯渲染（图标+控件），无终端；B2 补真终端。中间态可接受（用户决策 ①）。
...
## ✅ F-QA（质量收敛与加固）— 收官（已合 main PR#25/26）
