...
| 5 | 🔄 **真用户 fork/CoW 压力回归**：✅ M5-1 真 `copy_page_table_level` CoW 标记测试（`896f93d`，消 M1 头号假测；挖出 level 语义坑已修）+ ✅ M5-2a `-smp forktest` CoW 压力门（`c398ffe`，真用户 fork+CoW 50 迭代 races=0，替手动 hack）／ ⏳ M5-2b 跨核 UAF（需 AP 进调度器，与 M3-2 test-mode halt 互斥，单独处理） | 🔄 | M5-1/M5-2a ✅ run-kernel-test-smp races=0；M5-2b 跨核待做 |
