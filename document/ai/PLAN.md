...
| 1 | PTY 核心纯逻辑 `pty.{hpp,cpp}`（master/slave 双 ring + slave 复用 `TTY` 行规范；master 写→slave 行规范→slave 读，slave 写→master 读，信号字符经 slave）+ host 单测 | ⏳ | host pty 单测（round-trip / 行规范 / 信号字符 / 满环） |
