...
| F8 | IPC 扩展 | M1 Pipe增强✅(BrokenPipe/SIGPIPE+真调度阻塞替 sti/hlt+O_NONBLOCK) M2 FIFO✅(FifoRegistry+FifoOps cloning+sys_mknod/mkfifo,2026-06-30) M3 Unix Socket✅(AF_UNIX Socket 适配器+内存 UnixRegistry+字节流 round-trip,2026-07-01 集成线待 push) M4 共享内存✅(SysV shmget/shmat/shmctl/shmdt+ShmRegistry 固定表+mapcount 共享页,两进程映射同物理页,2026-07-01 待 push) M5 epoll⏳ | CV/PTY/shm/epoll |
...
| F12 | 开发者生态（**用户决策:砍 Lua/TinyCC 自建,GCC 自举**） | M1 GDB/KALLSYMS✅(KALLSYMS 合 F-INFRA,GDB 待) M2 GCC 自举⏳(GCC+binutils 在 CinuxOS 上跑,编 .c 产 ELF) M3 自举闭环⏳(在 CinuxOS 编 CinuxOS) M4 包管理+编辑器⏳ | 现成 GCC/binutils 运行→自举编译→日常可用 |
