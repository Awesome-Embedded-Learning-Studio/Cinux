...
| F-VERIFY | 动态验证与并发检测基建（测试保真度/SMP 覆盖/并发检测/机制回读/故障可观测） | 🔄 立项(2026-06-27):audit(15-agent workflow 抽 165 调试时间坑+审 5 维基建)结论——14 维静态债务审计量错轴(代码对≠代码被跑到/机制真生效/崩了看得懂)。165 坑根因 top:机制没真生效 27/并发没压到 22/基建≠生产 19/规格看错 17/内存 UAF 14。**全做 M0-M6**,重锤 SMP+并发,并补测试矩阵盘点+测试代码整理(现在测试太乱)。M0 零风险赢点(RUN_ALL_TESTS 虚报修/unsigned-overflow UBSAN/grep-a/kMaxCpus static_assert/test_f9 OSFXSR 回读/check_memory_layout 进CI)。M1 测试矩阵盘点(`test-matrix.md`:子系统×{host-unit/host-integration真码/QEMU-kernel/QEMU-SMP/ring3/机制回读}→覆盖/缺/假测,平行 debt.md)。M2 测试代码整理(CurrentTaskGuard/MockAS 共享 util、消 0x1234 假 CoW、镜像副本改链接真码、拆 flat main_test)。M3 **SMP 测试唤醒基建**(测试内核镜像 boot_aps,AP 进受控 worker,run-kernel-test-smp 不再空转)。M4 **并发检测**(host CINUX_HOST_TSAN + 内核 KCSAN-lite opt-in,补 lockdep 看不见的原始数据竞争)。M5 **真用户 fork/CoW 压力回归**(防 F10 四修回归+继承 AS execve,-smp 真压力复验)。M6 **故障可观测增强**(handle_pf CoW PTE/mapcount 解码/用户栈 hexdump/#PF debugcon 首故障/page owner tag)。详见 PLAN「🔄 F-VERIFY」段 + audit memory `debugging-audit-dynamic-coverage-gap` |

## 当前焦点
**F-VERIFY 动态验证与并发检测基建 🔄 立项（2026-06-27）**：横切里程碑（与 FO/F-INFRA/F-QA/F-CLN 同档）。起源 audit（15-agent workflow 抽 165 调试时间坑 + 审 5 维基建）：14 维静态债务审计量「代码对不对」，但所有超时调试都是「代码有没有真被跑到 / 机制真没真生效 / 崩了看不看得懂」——另一根轴。**全做 M0-M6，重锤 SMP+并发**，并补**测试矩阵盘点**（现在测试太乱、没人知道到底覆盖了什么）+**测试代码整理**（框架 bug/共享 util/0x1234 假 CoW/镜像副本）。已坐实三大基建缺口：run-kernel-test-smp 空转（不 boot_aps）、fork CoW 测试用 0x1234 哨兵是死代码、全仓零数据竞争检测器（CINUX_HOST_TSAN 只是一行注释）。先改 ROADMAP/PLAN 立项，等用户确认再开分支。详见 PLAN「🔄 F-VERIFY」段。
