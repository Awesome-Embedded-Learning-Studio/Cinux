...
## 当前焦点
**F2-M3 ✅ 完成**（brk syscall：懒 brk + execve brk_initial + Heap VMA，722/0 + 实机启动不炸，详见 `PLAN.md`/`document/notes/2026-06-17-f2-m3-brk.md`）。**F2-M4 Page Cache 🔄 进行中**（propose 已确认：最小 MVP = cache + file-mmap demand-read 读路径；从干净 main 开；详见 `PLAN.md`）。下一焦点 F2-M4。
