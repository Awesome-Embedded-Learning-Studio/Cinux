...
## 当前焦点
**F5-M1 ✅ 完成**（AHCI DMA 迁移：ahci.cpp ad-hoc DMA → M3 DmaPool/PrdtBuilder + ATA IDENTIFY + FLUSH CACHE，闭环 block device→AHCI 栈，705/0。详见 `PLAN.md`）。下一焦点待定——F5-M2 VirtIO / F2 内存管理 / F3 进程，或 `/milestone` 拆批。
