...
| F4 | SMP 多核 | M1 ACPI✅ M2 APIC✅ M3 AP启动✅ M4 多核调度✅ M5 同步原语🔄(R3✅) | ACPI+LAPIC+IOAPIC+PIC→APIC切换 ✅；per-CPU 架构(GS/gdt_blocks/swapgs)✅；IPI+trampoline+AP boot ✅(-smp 2 双核 online+idle)；**M4 多核调度 ✅**(per-CPU idle+runq+reschedule IPI+prepare-to-wait+真 user-task 迁移;-smp 2 AP 真跑 user task/无 GP)；**M5 R3 ✅**(SharedCwd/SharedSigActions 原子 refcount) |
