...
## 当前焦点
**F1-M3 🔄 进行中**（DMA 基础设施：`DmaBuffer` + `DmaPool` + `PrdtBuilder`，收编 ad-hoc DMA，归属 `kernel/drivers/dma/`。详见 `PLAN.md`）。M2 ✅ 完成（内核日志/dmesg：`ConcurrentRingBuffer` + `KernelLog` + `sys_dmesg` + exception 迁 klog_*）。
