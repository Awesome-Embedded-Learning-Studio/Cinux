...
| F7 | 网络协议栈 | M1 以太网✅ M2 ARP✅ M3 IPv4/ICMP✅（2026-06-26 真 ping 10.0.2.2 干通,底子优先 L0-L3:netdev 抽象+loopback 确定性打底→e1000 收尾） M4 UDP✅（2026-06-30,协议层+端口多路复用+Ipv4Module L4 proto 表(ICMP 迁入);host 单测+loopback round-trip+e1000 TX smoke,feat/f7-m4-udp 已合 main PR#47） M5 TCP✅（2026-06-30,TcpModule L4Handler proto=6:wire+伪首部校验和+状态机(三次握手/序号-ACK/四次挥手/RST);host 单测 15 例+内核 loopback 端到端(握手+数据+挥手)+e1000 TX smoke+对抗负测;最小可用无重传/窗口/拥塞,Socket 留 M6,worktree-f7-m5-tcp 已合 main PR#53） M6 Socket🔄（2026-06-30 立项,worktree-f7-m6-socket:Socket=InodeOps 适配器抄 PTY/pipe,TcpModule/UdpModule 纯协议层不动 + 生产 add_l4 注册 TCP/UDP + 挂 loopback;阻塞用 prepare_to_wait/schedule_blocked 不需 timer;最小可用无重传/RTO/窗口/拥塞 defer） | TCP/IP+Socket API |
| F8 | IPC 扩展 | M1 Pipe增强✅(BrokenPipe/SIGPIPE+真调度阻塞替 sti/hlt+O_NONBLOCK) M2 FIFO✅(FifoRegistry+FifoOps cloning+sys_mknod/mkfifo,2026-06-30) M3 Unix Socket⏳ M4 共享内存⏳ M5 epoll⏳ | CV/PTY/shm/epoll |
...
## 当前焦点
**🚩 F7-M6 Socket API 🔄（2026-06-30 立项，worktree-f7-m6-socket，从干净 main `f1f29aa`）**：网络线走到「可用」——socket fd 走 InodeOps 适配器（抄 PTY/pipe，sys_read/write/ioctl/close 零改动），TcpModule/UdpModule 纯协议层不动，per-socket RX 环 + 阻塞 + accept 队列放 Socket 适配器（挂 listener 缝），生产 net_init `add_l4` 注册 TCP/UDP + 挂 loopback。5 批（B0 立项 / B1 底座 / B2 UDP socket ‖ B3 TCP socket / B4 demo+收官）。**最小可用验收**：loopback 内核 echo 回归门 + musl 静态 socket 程序；TCP 重传/RTO/窗口/拥塞 defer（需内核 timer=F5-M4 follow-up，硬前置非 M6 活）。checkpoint push 不合入，整弧做完才 PR。详见 PLAN「🔄 F7-M6 Socket」段 + [todo f7-network/05-socket.md](todo/f7-network/05-socket.md)。
...
- 🚩 **F7-M6 Socket API 🔄 进行中**（worktree-f7-m6-socket，2026-06-30）：Socket=InodeOps 适配器抄 PTY/pipe + 生产 net_init `add_l4` 注册 TCP/UDP + 挂 loopback；详见上方焦点段 + [todo f7-network/05-socket.md](todo/f7-network/05-socket.md)。
