...
| F10 | 用户态运行时 | **M1 musl 静态移植🔄（2026-06-26 立项，砍自建 libc，直接移植 musl）** M2 ELF动态链接✅（2026-06-30 收官，用 musl ldso，kernel 做 PT_INTERP/interp 加载/auxv，不自建 loader；CinuxOS 跑通 musl 动态 hello） M3 TTY✅（Phase 1：行规范+阻塞读+ioctl TCGETS/TCSETS/TIOCGWINSZ+信号,feat/f10-m3-tty 已合 main PR#46） M4 CFBox+init⏳ | musl libc/sysroot/musl-gcc/CFBox；详见 PLAN「🔄 F10-M1」 |
...
## 当前焦点
**F10-M3 TTY Phase 2（PTY / 伪终端）🔄 立项（2026-06-30，feat/f10-m3-pty，从干净 main a134cb7）**：Phase 1 显式推迟到 DevFS 的部分——PTY master/slave 对 + `/dev/ptmx` + `/dev/pts/N` + `/dev/tty` + TIOCSCTTY + `/dev/console` 接真 TTY。解锁条件已满（DevFS PR#48 / TTY Ph1 PR#46 / session-pgrp F3-M3 全在 main）。6 批（B0 立项+清文档债 / B1 PTY 核心纯逻辑+host 单测 / B2 InodeOps ioctl/open 接口缝+sys_ioctl fd→inode 派发 / B3 DevFS PTY 节点 / B4 TIOCSCTTY+controlling tty+/dev/tty / B5 端到端 smoke+收官）。范围栅栏：不做 TIOCSTI/流控/select-poll；slave 复用现成 TTY 行规范；用户态真 shell 闭环留 F10-M4。详见 PLAN「🔄 F10-M3 TTY Phase 2」段。
