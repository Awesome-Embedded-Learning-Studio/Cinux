...
| F12 | 开发者生态（**用户决策:砍 Lua/TinyCC 自建,GCC 自举**） | M1 GDB/KALLSYMS✅(KALLSYMS 合 F-INFRA,GDB 待) M2 GCC 自举🔄(批4-a as+ld 自举 ✅ 2026-07-02:glibc 动态 ELF 跑通,as+ld 编 hello + ./hello 自举 PASS;修 file-backed mmap CoW/mapcount + PageCache write invalidation + ext2 unlink + uaccess lazy fault + libc link artifacts;B4-b cc1+headers→B4-c gcc hello.c 完整自举待续) M3 自举闭环⏳(在 CinuxOS 编 CinuxOS) M4 包管理+编辑器⏳ | 现成 GCC/binutils 运行→自举编译→日常可用 |
...
| F-USABILITY | 可用性验证(Buildroot rootfs + gcc/g++ 冒烟 + CI 可用性测试) | 🔄 立项(2026-07-02,feat/f-usability,从 main `6ba4a88`):从"造 rootfs 夹具"升级"证明 CinuxOS 真能跑真实 Linux 用户态"。Buildroot external+busybox+musl 出 rootfs 骨架(强缓存,PR 内编),GCC **全拷贝不编译**(`tools/gcc-toolchain/extract.sh` 已落地 PR#62),overlay 叠加打包;CI 进 shell 跑真实工具作可用性测试;gcc 冒烟已闭环(CinuxOS 上 `gcc hello.c→./a.out` 输出 Hello),g++ 留批4。99 syscall 底子够(**wait4 实际在 slot 61** 命名 SYS_waitpid,仅缺 dup3/pipe2/recvmsg/sendmsg 兜底);busybox init PID1 + cc1→as→ld→./hello 已跑通。手搓 tools/musl/ 并存留快反馈。6 阶段:0 立项→1 验证+接管→2 可用性测试→3 gcc✅→4 g++→5 扩包。详见 PLAN「🔄 F-USABILITY」段 |
