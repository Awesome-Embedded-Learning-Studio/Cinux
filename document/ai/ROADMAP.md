...
|---|------|--------------------|---------|
| F1 | 内核基础设施 | M0 ✅(类型 Cinux-Base 就绪 + ErrorOr 消费迁移: FS 层批1/2a/2b✅ + syscall→errno 批4✅); M1 RingBuffer ⏳; M2 日志 ⏳; M3 DMA ⏳; M4 块设备 ⏳ | ErrorOr/StringView/Span/IBlockDevice/dmesg/DMA Pool |
...
## 当前焦点
**F1-M0 ✅ 完成**（ErrorOr 类型 + 消费迁移 + syscall→errno 全链路贯通，详见 `PLAN.md`）。下一焦点待定——F1-M1 RingBuffer（自然延续）或另行 `/milestone` 拆批。
