...
|---|------|--------------------|---------|
| F1 | 内核基础设施 | M0 ✅(类型 Cinux-Base 就绪 + ErrorOr 消费迁移: FS 层批1/2a/2b✅ + syscall→errno 批4✅); M1 RingBuffer消费迁移✅(pipe+keyboard复用Cinux-Base); M2 日志🔄(KernelLog+dmesg); M3 DMA ⏳; M4 块设备 ⏳ | ErrorOr/StringView/Span/IBlockDevice/dmesg/DMA Pool |
...
## 当前焦点
**F1-M2 🔄 进行中**（内核日志/dmesg：Cinux-Base `Logger` 就绪，增量 `ConcurrentRingBuffer` + `KernelLog` + kprintf 桥接 + `sys_dmesg`。详见 `PLAN.md`）。
