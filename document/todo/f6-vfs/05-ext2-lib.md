...
> - **T3 host 测试**：✅ B1a/B1b/B2 + block_buf_ follow-up —— `ext2_host_pal.cpp`（host PAL：kprintf/kvprintf/kpanic→vprintf/abort、kmalloc/kfree→aligned_alloc+zero、sink noop；**禁 link kernel 那份** kprintf.cpp/slab.cpp，吃 serial/inline-asm/PMM）+ `ext2_host_link`（冒烟：RAMBlockDevice→Ext2 ctor→mount→dtor，证 host 编链通，M6 关键关卡）+ `ext2_host`（真逻辑 ASAN：mount 镜像/readdir/多级 lookup/read/create/write/readback/unlink）+ `ext2_concurrent`（TSAN 并发 alloc/free + 4×200 lookup）。预制 `test/data/ext2_test.img`（64KB，`mke2fs -d` 填 /etc/motd + /hello.txt，.gitignore `!test/data/ext2_test.img` 例外）。
...
>
> **留续（非 M6 阻塞）**：
> - ✅ ⭐ **ext2 `block_buf_` SMP race（本次）**：`lookup_in_dir` 改 per-call `KmBuf` + `read_block(block,dst)`，在目录块循环内复用调用私有 scratch；`symlink` 改调用私有 `KmBuf` + `write_block(block,src)`；mount-only 共享 buffer 保留并标明 single-threaded。加回 4 线程 × 200 次并发 lookup TSAN 回归，每个成功 inode 配对 `inode_unref`。验证：TSAN 2/2 零 warning + 两 leg 937/937 + test_host 66/66 + ASAN 1/1。详见 [note](../../notes/2026-07-10-f6-m6-blockbuf-race.md)。
