...
- `Z`（修订号）：向后兼容的缺陷修复
- **仅在 main 分支上打版本 Tag**
- `v1.0.0` 标志跨过教程期：API 稳定，SMP 多核 / TCP/IP+Socket / GUI 用户态化 / gcc·g++ 自举 / musl 动态链接 / 安全机制（SMEP·SMAP·ASLR·Canary）均已落地，作为对外首个可发布版本；此后 `v1.x` 按语义化版本演进（UEFI、ext4 写、epoll、自举闭环等推 `v1.1+`）。
