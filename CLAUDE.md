...
- 验证用 `cmake --build build --target run-kernel-test -j$(nproc)`（QEMU 真内核 ~662 项）；`run`/`make run`（GUI 无断言）非验证。**QEMU 易挂死（panic 死循环/死锁不返回）——验证与冒烟一律 `timeout 40` 包裹**：`timeout 40 cmake --build build --target run-kernel-test -j$(nproc)` / `timeout 40 make run`。**run-kernel-test 不编译 `test/unit/` host 单测**，改公共接口/InodeOps/mock 后 push 前补 `cmake --build build -j$(nproc)` 全量（CI 另跑 `make test_host`，详见 DIRECTIVES L5）。
- 一批一 commit 一验证；绿才提交。
