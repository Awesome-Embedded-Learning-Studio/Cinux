/**
 * @file visor/host/fake_host_main.cpp
 * @brief Standalone host-neutral smoke harness for the visor core
 *
 * Drives cinux::gui::visor_pump() with a hand-filled visor_host table -- NO
 * kernel, NO framebuffer, NO real input. Built only when visor/ is the build
 * root (`cmake -S visor -B visor/build && ctest --test-dir visor/build`). If
 * this links and runs green under a standard hosted compiler, the visor core
 ...
 * the core runs unchanged. visor body (widgets, compositor, M0-M9) is developed
 * against this in a separate project.
 *
 * NOT built inside the CinuxOS kernel tree -- the kernel has its own in-QEMU
 * tests under kernel/test/ (test_visor_dirty / _region / _swraseter).
 */

#include <cstdint>
#include <cstdio>

#include "visor_host.h"      // visor_host, visor_frame, visor_rect, visor_pixel_format
#include "visor_pump.hpp"    // cinux::gui::visor_pump
#include "visor_region.hpp"  // visor::Rect, visor::rect_intersect, visor::Region
...
// ---- per-test render behaviour (set before each visor_pump call) ----
enum class RenderMode {
    Idle,
    OneRect
};
RenderMode g_render_mode = RenderMode::Idle;
visor_rect g_test_rect{};
uint32_t   g_stage[4 * 4];  // 4x4 staging buffer the dirty frame renders "into"

// ---- fake host callbacks (the table fill a real host swaps) ----
bool fake_poll_event(void* /*ctx*/, visor_event_header* /*out*/, uint16_t /*out_cap*/) {
    return false;  // no input queued -- the drain loop does nothing
}

void fake_dispatch_event(void* /*ctx*/, const visor_event_header* /*ev*/, const void* /*payload*/) {
    // no-op: nothing to apply in a fake host
}

void fake_render_frame(void* /*ctx*/, visor_frame* frame) {
...
void reset(RenderMode mode, visor_rect r) {
    g_render_mode = mode;
    g_test_rect   = r;
    g_flush_count = 0;
}
...
int run() {
    // 1. NULL host is a safe no-op (defensive discipline of visor_pump).
    cinux::gui::visor_pump(nullptr);

    // 2. Idle frame: render_frame reports count==0 -> pump flushes nothing.
    reset(RenderMode::Idle, {});
    visor_host h = make_host();
    ...
    reset(RenderMode::OneRect, visor_rect{1, 2, 3, 4});
    h = make_host();
    cinux::gui::visor_pump(&h);
    ...
    CHECK(g_flushed[0].format == VISOR_PIX_XRGB8888, "flush format %d (expected XRGB8888)",
          g_flushed[0].format);

    // 4. Region algebra ports (pure logic, no host): intersect + bounded Region add.
    visor::Rect a{0, 0, 10, 10}, b{5, 5, 15, 15};
    visor::Rect isect = visor::rect_intersect(a, b);
    CHECK(isect.x0 == 5 && isect.y0 == 5 && isect.x1 == 10 && isect.y1 == 10,
          "rect_intersect {%d,%d,%d,%d} (expected 5,5,10,10)", isect.x0, isect.y0, isect.x1,
          isect.y1);

    visor::Region reg;
    reg.add(visor::Rect{0, 0, 4, 4});
    reg.add(visor::Rect{10, 10, 14, 14});
    CHECK(reg.count() == 2, "region count %u (expected 2)", reg.count());

    std::printf(
        "visor_host_smoke: OK (null-host safe + idle skip + dirty flush + region "
        "algebra)\n");
    return 0;
}
