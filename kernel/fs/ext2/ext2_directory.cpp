...
// ============================================================
// F-ECO batch 2 dirent-op overrides (block B).
// Thin wrappers around Ext2::symlink/link/rename, translating the
// bool/pointer result into an ErrorOr<void> (mirror create/unlink).
// ============================================================
...
// ============================================================
// F-ECO batch 2 block B: Ext2 symlink / link / rename
// (reuse the create/mkdir/unlink primitives: alloc_inode,
// alloc_block, add_dir_entry, remove_dir_entry, lookup_in_dir).
// ============================================================
...
Inode* Ext2::symlink(uint32_t parent_ino, const char* name, uint32_t name_len, const char* target) {
    ...
    // Store the target string in the first data block (i_block[0]); block A's
    // readlink reads it back from there, sized by i_size.
    uint32_t data_blk = alloc_block();
    if (data_blk == 0) {
...
bool Ext2::rename(uint32_t src_dir_ino, const char* src_name, uint32_t src_len,
    ...
    // When src==dst the single write above (dst_dir_ino == src_dir_ino) has
    // already persisted the same buffer, no second write needed.
