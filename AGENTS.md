...
- 本地复现 GitHub Actions/CI 优先用 `scripts/act_ci_job.sh [job]`（默认 `gcc-smoke`）：脚本会同步到 `/tmp/cinux-act-<job>`、移除 checkout 步骤避免误伤真实 worktree、默认走 WSL/Docker Desktop 代理 `http://host.docker.internal:7890`。
- 一批一 commit 一验证；绿才提交。
